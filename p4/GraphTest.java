...
public class GraphTest {
  ...
  public final void addVertexShouldAddUniqueVertices() {
    ...
    for (String itrVertex1 : vertices) {
      ...
      for (String itrVertex2 : this.graph.getAllVertices()) {
        verticesCount++;
        assertEquals("added vertex value present in graph", true,
            addedVertices.contains(itrVertex2));
      }
  ...
  public final void addEdgeIfAnyOfTheVerticesDoesNotExist() {
    ...
    List<String> notToBeAddedVertices =
        new ArrayList<>(vertices.subList(breakIndex, breakIndex + 2));
    for (String itrVertex : toBeAddedVertices)
      this.graph.addVertex(itrVertex);

    assertEquals("return value if vertex1 doesn't exist", false,
        this.graph.addEdge(notToBeAddedVertices.get(0), toBeAddedVertices.get(0)));
    assertEquals("return value if vertex2 doesn't exist", false,
        this.graph.addEdge(toBeAddedVertices.get(0), notToBeAddedVertices.get(0)));
    assertEquals("return value if both vertex1 and vertex2 doesn't exist", false,
        this.graph.addEdge(notToBeAddedVertices.get(0), notToBeAddedVertices.get(1)));
  ...
  public final void addEdgeIfBothVerticesExist() {
    ...
    assertEquals("return value if vertex1 equals vertex2", false,
        this.graph.addEdge(vertices.get(0), vertices.get(0)));
    assertEquals("return value if vertex1 does not equal vertex2", true,
    ...
    for (String itrVertex : this.graph.getAllVertices()) {
      ...
      for (String itrNeighbor : this.graph.getNeighbors(itrVertex)) {
        ...
        if (itrVertex.equals(vertices.get(0)))
          assertEquals(String.format("neighbor of vertex=%s", itrVertex), vertices.get(1),
              itrNeighbor);
        else if (itrVertex.equals(vertices.get(1)))
          assertEquals(String.format("neighbor of vertex=%s", itrVertex), vertices.get(0),
              itrNeighbor);
      }
  ...
  public final void removeEdgeIfAnyOfTheVerticesDoesNotExist() {
    ...
    assertEquals("return value if vertex1 doesn't exist", false,
        this.graph.removeEdge(notToBeAddedVertices.get(0), toBeAddedVertices.get(0)));
    assertEquals("return value if vertex2 doesn't exist", false,
        this.graph.removeEdge(toBeAddedVertices.get(0), notToBeAddedVertices.get(0)));
    assertEquals("return value if both vertex1 and vertex2 doesn't exist", false,
        this.graph.removeEdge(notToBeAddedVertices.get(0), notToBeAddedVertices.get(1)));
  ...
  public final void removeEdgeIfBothVerticesExist() {
    ...
    assertEquals("return value if vertex1 equals vertex2", false,
        this.graph.removeEdge(vertices.get(0), vertices.get(0)));
    assertEquals("return value if vertex1 does not equal vertex2", true,
        this.graph.removeEdge(vertices.get(0), vertices.get(1)));
    ...
    for (String itrVertex : this.graph.getAllVertices()) {
      ...
      for (String itrNeighbor : this.graph.getNeighbors(itrVertex)) {
        ...
        if (itrVertex.equals(vertices.get(1)))
          assertEquals(String.format("neighbor of vertex=%s", itrVertex), vertices.get(2),
              itrNeighbor);
  ...
  public final void isAdjacentIfAnyOfTheVerticesDoesNotExist() {
    ...
    assertEquals("return value if vertex1 doesn't exist", false,
        this.graph.isAdjacent(notToBeAddedVertices.get(0), toBeAddedVertices.get(0)));
    assertEquals("return value if vertex2 doesn't exist", false,
        this.graph.isAdjacent(toBeAddedVertices.get(0), notToBeAddedVertices.get(0)));
    assertEquals("return value if both vertex1 and vertex2 doesn't exist", false,
        this.graph.isAdjacent(notToBeAddedVertices.get(0), notToBeAddedVertices.get(1)));
  }
  ...
  public final void isAdjacentIfBothVerticesExist() {
    ...
    assertEquals("return value if vertex1 equals vertex2", false,
        this.graph.isAdjacent(vertices.get(0), vertices.get(0)));
    assertEquals(
        "return value if vertex1 does not equal vertex2, both are disconnected and vertex1 has a neighbor",
        false, this.graph.isAdjacent(vertices.get(0), vertices.get(3)));
    assertEquals(
        "return value if vertex1 does not equal vertex2, both are disconnected and vertex2 has a neighbor",
        false, this.graph.isAdjacent(vertices.get(3), vertices.get(0)));
    assertEquals(
        "return value if vertex1 does not equal vertex2, both are disconnected and both have a neighbor",
        false, this.graph.isAdjacent(vertices.get(2), vertices.get(0)));
    assertEquals("return value if vertex1 does not equal vertex2 and both are connected", true,
        this.graph.isAdjacent(vertices.get(0), vertices.get(1)));
  }
  ...
  public final void removeVertexReturnsNull() {
    ...
    assertEquals("return value if vertex passed to removeVertex does not exist in graph", null,
        vertex);
  ...
  public final void removeVertexReturnsNonNull() {
    ...
    assertEquals("return value if vertex passed to removeVertex is disconnected", vertices.get(2),
        vertex);

    vertex = this.graph.removeVertex(vertices.get(0));
    assertEquals("return value if vertex passed to removeVertex is connected to another vertex",
        vertices.get(0), vertex);
    ...
    for (String itrVertex : this.graph.getAllVertices()) {
      ...
      assertEquals(String.format("presence of vertex=%s", itrVertex), false,
          itrVertex.equals(vertices.get(0)));
      assertEquals(String.format("presence of vertex=%s", itrVertex), false,
          itrVertex.equals(vertices.get(2)));
    }
